...
def main():
        ...
        print("❤❤"*40)
        print("**"*40)
        print("Welcome to password Locker Saves you password and the hustle of forgeting them, It also generates you password")
        ...
        if short_codes == "g":
            print(
                "Enter the length that you would like your password to have. Recommended length:)5")
            limit = int(input())
            print('\n')
            print("**"*15)
        ...
        if vUser == name and vPassword == password:
            ...
            while True:
                ...
                if short_codes == "cp":
                    ...
                    print(
                        f"{name} Please enter the account name you need to be saved eg-Instagram | Facebook")
                    thisAccout = input();
                    ...
                    if display_passwords():
                        ...
                        for account in display_passwords():
                            print(
                                f"{account.username} \t {account.password}............{account.email}")
                            print("\n")
                        print("--"*40)
                    else:
                        print("\n")
                    ...
                    if check_if_accountExist(delAccount):
                        for i in display_passwords():
                            if delAccount in i.username:
                                posit=display_passwords().index(i)
                                
                        # delete_account
                        display_passwords().remove(display_passwords()[posit])
                        print("\n")
